...
(rum/defc page-blocks-inner <
  {:did-mount (fn [state]
                ...
                state)}
  [page-e blocks config sidebar? _block-uuid]
  (when page-e
...
(rum/defc page-title-editor < rum/reactive
  ...
  (let [input-ref (rum/create-ref)
        ...
        blur-fn (fn [e]
                  ...
                  (cond
                    (or (= old-name @*title-value) (string/blank? @*title-value))
                    (reset! *edit? false)
...
(rum/defcs ^:large-vars/cleanup-todo page-inner < rum/reactive db-mixins/query mixins/container-id
  ...
  [state {:keys [repo page preview? sidebar? tag-dialog? linked-refs? unlinked-refs? journals?] :as option}]
        ...
        [:div.flex-1.page.relative.cp__page-inner-wrap
         ...
         (when-not preview?
           [:div.flex.flex-col.gap-8
            ...
            ;; referenced blocks
            (when-not (or tag-dialog? linked-refs? block?)
